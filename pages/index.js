...
const customSearchClient = {
  search(requests) {
    console.log("search", JSON.stringify(requests));
  ...
  searchForFacetValues(requests) {
    console.log("searchForFacetValues", JSON.stringify(requests));

    return fetch("http://localhost:3000/api/sffv", {
      method: "post",
      headers: {
        "Content-Type": "application/json"
      },
      // body: JSON.stringify({ requests: new_requests }),
      body: JSON.stringify({ requests })
    }).then((res) => res.json());
  }
};
